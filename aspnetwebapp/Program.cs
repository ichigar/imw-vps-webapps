...
string HtmlContainer(string content, string title) => $@"<!DOCTYPE html>
<html lang=""es"">
<head>
  <meta charset=""UTF-8"">
  <title>{title}</title>
...
string GetClientIp(HttpContext context)
{
    ...
    return context.Connection.RemoteIpAddress?.ToString() ?? "Desconocido";
}
...
app.MapGet("/", (HttpContext context) =>
{
    ...
    if (string.IsNullOrEmpty(userAgent)) userAgent = "Desconocido";

    var content = $@"<h1>Información del cliente</h1>
    ...
    return Results.Content(HtmlContainer(content, "ASP.NET Web App"), "text/html; charset=utf-8");
});

app.MapGet("/contact", () =>
{
    var content = @"<h1>Formulario de contacto</h1>
    <p class=""note"">
    ...
    return Results.Content(HtmlContainer(content, "Contacto - ASP.NET Web App"), "text/html; charset=utf-8");
});
...
app.MapFallback(() =>
{
    var content = @"<h1>404 - Página no encontrada</h1>
    <p><a href=""/"">Volver al inicio</a></p>";

    return Results.Content(HtmlContainer(content, "404 - ASP.NET Web App"), "text/html; charset=utf-8");
});

var port = Environment.GetEnvironmentVariable("PORT") ?? "3000";
app.Urls.Add($"http://0.0.0.0:{port}");

Console.WriteLine($"Servidor ejecutándose en http://localhost:{port}");
app.Run();
