# Aplicación Web Clojure Simple

Aplicación web minimalista en Clojure usando Ring, Compojure y Jetty.
...
├── src/
│   └── clojurewebapp/
│       └── core.clj      # Aplicación Ring/Compojure principal
├── deps.edn              # Dependencias (Ring, Compojure, Jetty)
├── scripts/
│   ├── 01_preparar_servidor.sh    # Instalar Java y Clojure CLI
│   ├── 02_preparar_entorno.sh     # Descargar dependencias (ejecutar tras cambios de código)
...
```bash
# 1. Instalar Java y Clojure CLI
sudo ./scripts/01_preparar_servidor.sh

# 2. Descargar dependencias de la aplicación
...
Cada vez que cambies el código fuente o las dependencias (deps.edn), ejecuta:
...
Este script:
- Descarga/actualiza las dependencias Clojure
...
# Ejecutar aplicación
cd clojurewebapp
PORT=3000 clojure -M -m clojurewebapp.core
```
...
```bash
PORT=8080 clojure -M -m clojurewebapp.core
```
...
- **Requiere**: sudo
- **Qué hace**: Instala Java (OpenJDK 17) y Clojure CLI (tools.deps)

### 02_preparar_entorno.sh
- **Cuándo ejecutar**: Cada vez que cambia el código fuente o el deps.edn
- **Requiere**: Ninguno (pero reinicia el servicio si existe)
- **Qué hace**: Descarga/actualiza las dependencias Clojure
...
```bash
# Ver estado
sudo systemctl status clojurewebapp

# Ver logs
sudo journalctl -u clojurewebapp -f

# Reiniciar
sudo systemctl restart clojurewebapp

# Detener
sudo systemctl stop clojurewebapp

# Iniciar
sudo systemctl start clojurewebapp
```
