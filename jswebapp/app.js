...
// Configurar trust proxy para obtener IP real del cliente
app.set('trust proxy', true);

// Middleware para parsear formularios
app.use(express.urlencoded({ extended: true }));

// Función helper para generar HTML con estilos comunes
function container(content, title = 'JS Web App') {
  return `<!DOCTYPE html>
<html lang="es">
<head>
  <meta charset="UTF-8">
...
// Ruta principal: información del cliente
app.get('/', (req, res) => {
  const clientIp = req.ip || req.connection.remoteAddress || 'Desconocido';
  ...
  res.send(container(content, 'JS Web App'));
});

// Ruta GET /contact: mostrar formulario
app.get('/contact', (req, res) => {
  ...
  res.send(container(content, 'Contacto - JS Web App'));
});

// Ruta POST /contact: procesar formulario
...
app.use((req, res) => {
  ...
  res.status(404).send(container(content, '404 - JS Web App'));
});
